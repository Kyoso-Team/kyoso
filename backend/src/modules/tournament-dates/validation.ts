...
const UpdateTournamentDates = v.pipe(
  v.record(
    v.union([v.literal('publishedAt'), v.literal('concludedAt'), v.literal('playerRegsOpenedAt'), v.literal('playerRegsClosedAt'), v.literal('staffRegsOpenedAt'), v.literal('staffRegsClosedAt')]),
    v.date()
  ),
  v.transform((input) => Object.fromEntries(Object.entries(input).map(([key, value]) => [key, value.getTime()])) as Record<keyof typeof input, number>),
  v.check(
    (input) => dateIsBefore(input.publishedAt, [
  ...
  v.check(
    (input) => dateIsBefore(input.playerRegsOpenedAt, [input.playerRegsClosedAt, input.concludedAt]) && dateIsAfter(input.playerRegsOpenedAt, [input.publishedAt]),
  ...
  v.check(
    (input) => dateIsBefore(input.staffRegsOpenedAt, [input.staffRegsClosedAt, input.concludedAt]) && dateIsAfter(input.staffRegsOpenedAt, [input.publishedAt]),
  ...
  v.check(
    (input) => dateIsBefore(input.playerRegsClosedAt, [input.concludedAt]) && dateIsAfter(input.playerRegsClosedAt, [input.publishedAt, input.playerRegsOpenedAt]),
  ...
  v.check(
    (input) => dateIsBefore(input.staffRegsClosedAt, [input.concludedAt]) && dateIsAfter(input.staffRegsClosedAt, [input.publishedAt, input.staffRegsOpenedAt]),
  ...
  v.check(
    (input) => dateIsAfter(input.concludedAt, [
      input.publishedAt,
      input.playerRegsOpenedAt,
      input.playerRegsClosedAt,
      input.staffRegsOpenedAt,
  ...
  v.transform((input) => Object.fromEntries(Object.entries(input).map(([key, value]) => [key, value ? new Date(value) : null])) as Record<keyof typeof input, Date | null>)
);
