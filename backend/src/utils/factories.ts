...
>(repositoryQuery: TQuery, errorMessage: string) {
...
export function createServiceFnFromRepositoryQueryAndValidation<
  ...
  TQuery extends
    | ((db: DatabaseClient, data: v.InferOutput<TSchema>, ...args: any[]) => Promise<any>)
    | ((redis: RedisClient, data: v.InferOutput<TSchema>, ...args: any[]) => Promise<any>)
>(schema: TSchema, repositoryQuery: TQuery, item: string, errorMessage: string) {
  return async (...args: Parameters<TQuery>): Promise<Awaited<ReturnType<TQuery>>> => {
    const db = args.shift();
