...
export const authRouter = elysia({
	...
	.get('/login', ({ cookie, query, redirect, cookieService }) => {
	...
	.post('/login/test', async ({ body, sessionToken, authenticationService }) => {
		if (sessionToken) {
		...
		(app) =>
			app
				...
				.onBeforeHandle(({ cookie, query, cookieService, path }) => {
					const storedState = cookieService.getOAuthState(cookie, path.split('/').at(-1) as 'osu' | 'discord');
				...
				.get(
					'/osu',
					async ({ query, cookie, redirect, headers, ipAddress, authenticationService, ipInfoService, cookieService, userService }) => {
						const osuTokens = await authenticationService.getOsuTokens(query.code);
						const osuUserId = authenticationService.getOsuUserIdFromAccessToken(osuTokens.accessToken);
						...
						if (osuUser) {
							...
							const sessionToken = await authenticationService.createSession(osuUser.userId, ipAddress, ipMetadata, headers['user-agent'] ?? null);
				...
				.get(
					'/discord',
					async ({ query, cookie, headers, redirect, ipAddress, authenticationService, cookieService, ipInfoService, osuService, discordService }) => {
						...
						const ipMetadata = await ipInfoService.getIpMetadata(ipAddress);
						await authenticationService.createSession(user.id, ipAddress, ipMetadata, headers['user-agent'] ?? null);
				...
				.get(
					...
					async ({ query, redirect, authenticationService, discordService }) => {
						const discordTokens = await authenticationService.getChangeAccountDiscordTokens(query.code);
						const discordUser = await discordService.getDiscordSelf(discordTokens.accessToken);
	...
	.get('/logout', async ({ query, cookie, redirect, authenticationService, cookieService }) => {
		const sessionToken = env.NODE_ENV === 'test' ? await authenticationService.getTestSessionToken() : cookieService.getSession(cookie);
		if (!sessionToken) {
